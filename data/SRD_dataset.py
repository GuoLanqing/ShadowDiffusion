...
class SRDDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.datatype == 'lmdb':
            ...
            if self.split == 'train':
                mask_name = self.sr_path[index].replace('train_A', 'train_B')
            else:
                mask_name = self.sr_path[index].replace('test_A', 'SRD_testmask')
            img_mask = Image.open(mask_name).convert("1")
            if self.need_LR:
